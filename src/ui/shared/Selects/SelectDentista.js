...
import DentistaApi from 'service/api/dentista.api';

class SelectDentista extends React.Component {
    ...
    componentWillMount() {
        ...
        DentistaApi.getAll({paginate: true}).then((items) => {
    ...
    retornar(e, val) {
        ...
        if (val && val.toString().length >= 3) {
            ...
            DentistaApi.getAll({ search: val }).then((items) => {
                items = items.map((x) => {
                    return {
                        text: x.nome,
                        value: x.id
